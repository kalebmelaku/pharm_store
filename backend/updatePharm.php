...
if (isset($_POST['updateBtn'])) {
    ...
    $now = date('Y-m-d');
    // echo $price;
    $update = $conn->query("UPDATE `meds` SET `name`='$name',`type`='$type',`price`='$price',`exdate`='$expDate' WHERE `med_id` = '$med_id'");
    if(!$update){
    ...
    if ($rs['amount'] > 0) {
        ...
        $delete = $conn->query("DELETE FROM `meds` WHERE `med_id` = '$med_id' ");
        if ($delete) {
