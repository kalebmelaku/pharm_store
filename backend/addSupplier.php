...
if (isset($_POST['nextForm'])) {
    ...
    for ($i = 1; $i <= $list; $i++) {
        ...
        $insert_temp = $conn->query("INSERT INTO `temp_meds`(`name`, `type`, `purchase_price`, `sell_price`, `quantity`, `exdate`, `invoice_no`) VALUES ('$name', '$type', '$price', '$sell_price', '$quantity', '$date', '$invoice_number')");
        if ($insert_temp) {
   ...
   // get all from temp
    ...
    $getTotal = $conn->query("SELECT SUM(purchase_price) AS `total` FROM `temp_meds` WHERE `invoice_no` = '$invoice_number'");
    ...
    while ($row = $getAll->fetch_assoc()) {
        ...
        if ($update) {
            ...
            $insert_meds = $conn->query("INSERT INTO `pharm_store`(`name`, `type`, `amount`, `purchase_price`, `sell_price`, `exdate`, `invoice_no`) VALUES (
                '$name', '$type', '$quantity', '$price', '$sell_price', '$exdate', '$invoice_number'
            )");
