...
if (isset($_POST['updateBtn'])) {
    ...
    $now = date('Y-m-d');
    $update = $conn->query("UPDATE `pharm_store` SET `name`='$name',`type`='$type',`amount`='$amount',`cost`='$price',`date`= '$now',`exdate`='$expDate' WHERE `id` = '$med_id'");
    if(!$update){
    ...
    $search = $conn->query("SELECT `amount` FROM `meds` WHERE `med_id` = '$med_id'");
    ...
    if ($rs['amount'] > 0) {
        ...
        if ($delete) {
            $conn->query("DELETE FROM `meds` WHERE `med_id` = '$med_id' ");
